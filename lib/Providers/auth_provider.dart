...
class AuthProvider with ChangeNotifier {
  ...
  Future<bool> login(
    ...
    // log('Response Status: ${response.statusCode}');  k9b66
  ...
  Future getRolesAndModules(BuildContext context) async {
    ...
    if (response['success'] == true && response['data'] != null) {
      ...
      try {
        ...
        // Filter to show only TP and TI (TERTIARY) roles
        // Only allow "TP" and "TS" roles
        final allowedRoleCodes = {"TP", "TS"};
        final filteredRoles = txUserRoleResponse.data
            .where((role) => allowedRoleCodes.contains(role.code))
            .toList();

        final filteredData = filteredRoles.map((role) {
          // For TP: filter for OPENIND, ASGNTRK, ALLIND
          // For TS: filter for TD only (per provided json sample)
          Set<String> allowedModules;
          if (role.code == "TP") {
            allowedModules = {"OPENIND", "ASGNTRK", "ALLIND"};
          } else if (role.code == "TS") {
            allowedModules = {"TD"};
          } else {
            allowedModules = {};
          }
          final filteredModules = role.modules
              .where((module) => allowedModules.contains(module.code))
              .toList();
